...
struct Physical_MM {
    ...
    struct Node {
        ...
        Node() = default;
        Node(vec4 position, vec4 velocity, Sphere3D sphere) : position(position), velocity(velocity), sphere(sphere) {}
    ...
    /*
    // ID system:
        0 - nodes.size()-1: node with title id_to_title[i] 
        nodes.size() - nodes.size()+mm.connections.size()-1 : mm.connections[nodes.size()-i]
    */
   std::vector<std::string> id_to_title; //same size as nodes
    ...
    Physical_MM(MM mm) : mm(mm) {
        ...
        for (auto& node : mm.nodes) {
            ...
            Node newNode(position, velocity, sphere);
            
            nodes[node.first] = newNode;
            id_to_title.push_back(node.first);
            
            collection.c.push_back(std::make_pair(id, &(nodes[node.first].sphere)));
            id++;
        }
        ...
        for (auto& connection : mm.connections) {
            Line3D line(nodes[connection.first].position, nodes[connection.second].position, 1.0f);
            lines.push_back(line);

            collection.c.push_back(std::make_pair(id, &lines[lines.size()-1]));
            id++;
        }
    }
    ...
    void render(sf::RenderWindow& window, Camera& camera) {
        ...
        // If our mouse hovers both a node and a connection, we want to prefer the node
        for (auto it = collection.c.rbegin(); it != collection.c.rend(); ++it) {
            ...
            if (shape->computeCollisionWithPoint(mousePos)) {
                if (it->first < nodes.size()) {  //NODE ALERT
                    selected_id = it->first;
                    break;
                } else { //Meh... a connection. Only add this if we have not found a connection before. Even then, continue the search for a node if possible
        ...
        if (selected_id == -1) selected_id = selected_id_connection;
    ...
    bool are_sizes_matching() const {
        return mm.nodes.size() == nodes.size() && mm.connections.size() == lines.size();
    }

    //*
    Physical_MM(std::string path) : Physical_MM([&]() {
    ...
    void save(std::string path) {
        ...
        try {
            ...
            if (!bin.is_open()) {
                throw std::runtime_error("Failed to open physics.bin for writing.");
            }
            ...
            for (const auto& [title, node] : nodes) {
                ...
                // We cast to float explicitly to ensure 4-byte size regardless of vec4 internal precision
                float coords[3] = { static_cast<float>(node.position.x), 
                                    static_cast<float>(node.position.y), 
                                    static_cast<float>(node.position.z) };
                bin.write(reinterpret_cast<const char*>(coords), sizeof(coords));
            }

            bin.close();

            // 4. Atomic Swap: Backup existing data, move temp to main, delete backup
            if (fs::exists(path)) {
